...
def parse_bad_subjects():
    url = "https://dnevnik.admin-smolensk.ru/journal-student-grades-action?mode=print"

    headers = {"Content-Type": "application/octet-stream",
               "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/136.0.0.0 Safari/537.36"}

    wanted_mark = input("Введите минимальную оценку, ниже которой вы не хотите иметь все четвертные оценки: ")
    while True:
        try:
            wanted_mark = int(wanted_mark)
            if wanted_mark < 2 or wanted_mark > 5:
                raise
            break
...
def get_marks_count_for_wanted(wanted, summa, count):
    if count == 0:
        return 3
    k = 0
    while round(summa / count) < wanted:
        k += 1
        summa += wanted
        count += 1
    return k
